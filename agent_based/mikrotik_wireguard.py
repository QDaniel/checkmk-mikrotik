...
# factory settings:
mikrotik_wireguard_factory_settings = {
}


# parse function
def parse_mikrotik_wireguard(string_table):
...
# discovery function
def discover_mikrotik_wireguard(section_mikrotik_wireguard,section_mikrotik_wireguard_peers):
    for iface in list(section_mikrotik_wireguard):
        yield Service(item = iface)
    for iface in list(section_mikrotik_wireguard_peers):
        yield Service(item='%s Peer %s' % (iface, iface))

# check function
def check_mikrotik_wireguard(item, params, section):
...
register.agent_section(
    ...
    parse_function      = parse_mikrotik_wireguard,
)
register.agent_section(
    name                = "mikrotik_wireguard_peers",
    parse_function      = parse_mikrotik_wireguard,
)
...
register.check_plugin(
    name                     = "mikrotik_wireguard",
    sections                 = ["mikrotik_wireguard","mikrotik_wireguard_peers"],

    service_name             = "Wireguard %s",
    discovery_function       = discover_mikrotik_wireguard,
    check_function           = check_mikrotik_wireguard,
    check_default_parameters = mikrotik_wireguard_factory_settings,
)
